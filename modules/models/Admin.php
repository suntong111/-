...
class Admin extends ActiveRecord
{
    ...
    public function rules(){
         return [
             ['adminuser','required','message'=>'管理账号不能为空','on'=>['login','seekpass','changepass','adminadd','changeemail']],
             ...
             ['adminemail','vaildateEmail','on'=>'seekpass'],
...
//             die;
     ...
     public function seekPass($data){
         ...
         if ($this->load($data) && $this->validate()){
            ...
            $mailer->setFrom('stloveywj@163.com');
            $mailer->setTo($data['Admin']['adminemail']);
            $mailer->setSubject("找回密码");
//             $mailer->setTextBody("找回密码");
            if ($mailer->send()){
                return true;
            }
         }
     ...
     public function changeemail($data){
         ...
         if ($this->load($data) && $this->validate()){
          return (bool)$this->updateAll(['adminemail'=>$this->adminemail],'adminuser = :user',[':user'=>$this->adminemail]);
         }
         return false;
     }
}